...
// solution
    void solve()
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            for (int j = 0; j < n; ++j)
            {
                if (grid[i][j] == '.')
                    continue;
                if (check(grid, i, j, n, 1) == sum ||
                    check(grid, i, j, n, 2) == sum ||
                    check(grid, i, j, n, 3) == sum ||
                    check(grid, i, j, n, 4) == sum)
                {
                    std::cout << yes <<endl;
                    return;
                }
    ...
    int check(std::vector<std::string> &grid, int x, int y, int n, int mode)
    {
        int sum = 0;
        int count = 0;
