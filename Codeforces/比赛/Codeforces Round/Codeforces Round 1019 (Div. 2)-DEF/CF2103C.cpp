...
// solution
	void solve()
	{
		...
		// V V ~
		vector<int> pres_sum(n);
		partial_sum(as.begin(), as.end(), pres_sum.begin());
		vector<int> sufs_max(n);	// 前缀和的后缀最大值
		partial_sum(pres_sum.rbegin() + 1, pres_sum.rend(), sufs_max.rbegin() + 1, [](int a, int b)
		{
			return a > b ? a : b;
		});
		for (int i = 0; i < n - 2; ++i)
		{
			if (pres_sum[i] >= 0 && sufs_max[i+1] - pres_sum[i] >= 0)
			{
				cout << "YES" << endl;
				return;
			}
		...
		// ~ V V
		vector<int> sufs_sum(n);
		partial_sum(as.rbegin(), as.rend(), sufs_sum.rbegin());
		vector<int> pres_max(n);	// 后缀和的前缀最大值
		partial_sum(sufs_sum.begin() + 1, sufs_sum.end(), pres_max.begin() + 1, [](int a, int b)
		{
			return a > b ? a : b;
		});
		for (int i = n - 1; i - 2 >= 0; --i)
		{
			if (sufs_sum[i] >= 0 && pres_max[i-1] - sufs_sum[i] >= 0)
			{
				cout << "YES" << endl;
				return;
			}
