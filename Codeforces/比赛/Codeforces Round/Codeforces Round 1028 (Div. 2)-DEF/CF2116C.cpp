...
// solution
public:
	static void Solve(vector<vector<int>> &gs)
	{
		int n;
		cin >> n;
		vector<int> as(n + 1);
		...
		int g = accumulate(as.begin() + 1, as.end(), as[1], [&gs](int a, int b){return gs[a][b];});
		// int g = accumulate(as.begin() + 1, as.end(), as[1], [&gs](int a, int b){return gcd(a, b);});

		int count_g = std::count(as.begin() + 1, as.end(), g);
		if (count_g)
		{
			cout << n - count_g << endl;
			return;
		...
		int m = *max_element(as.begin() + 1, as.end());
		vector<int> dp(m + 1, 1e4);
		for (int i = 1; i <= n; ++i)
		{
			dp[as[i]] = 0;
			for (int j = 1;j <= m; ++j)
...
int main()
{
	...
	vector<vector<int>> gs(5001, vector<int>(5001));
