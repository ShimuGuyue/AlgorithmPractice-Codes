...
// function
public:
	static void Solve(vector<int64_t> &pres)
	{
		int n;
		cin >> n;
		vector<int> as(n);
...
int main()
{
	...
	if (ShimuGuyue::MULTIPLE_TESTS)
		cin >> t;
	vector<int64_t> primes = Euler(1e7);
	vector<int64_t> pres(primes.size());
	partial_sum(primes.begin(), primes.end(), pres.begin());
	while (t--)
	{
		ShimuGuyue::Solve(pres);
		#ifdef SHIMUGUYUE
		if (ShimuGuyue::MULTIPLE_TESTS) 
			cout << "-------------------------" << endl;
		#endif
