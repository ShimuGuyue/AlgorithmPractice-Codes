...
// solution
	void solve()
	{
		...
		auto swapab = [&as, &bs, &indexs_a, &ans](int i, int j)
		{
			if (i == j)
				return;
			swap(as[i], as[j]);
			swap(bs[i], bs[j]);
			swap(indexs_a[as[i]], indexs_a[as[j]]);
		...
		if (n % 2)
			swapab(index_equal, n / 2 + 1);
		for (int i = 1; i <= n / 2; ++i)
		{
			swapab(indexs_a[bs[i]], n - i + 1);
		}
