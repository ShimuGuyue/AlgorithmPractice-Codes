        ...
        for (int i {1}; i <= n; ++i)
        {
            for (int j {1}; j * j <= as[i]; ++j)
            {
                ...
                if (j != 1)
                {
                    ++factors[j];
                    if (factors[j] != i)
                        ans = std::max(ans, factors[j]);
                }
                if (j * j != as[i])
                {
                    ++factors[as[i] / j];
                    if (factors[as[i] / j] != i)
                        ans = std::max(ans, factors[as[i] / j]);
                }
            }
            for (int j {1}; j * j <= as[i - 1]; ++j)
            {
                if (as[i - 1] % j)
                    continue;
                if (j != 1)
                ...
                if (j * j != as[i - 1])
                {
                    if (factors[as[i - 1] / j] != i)
                        ans = std::max(ans, factors[as[i - 1] / j]);
                }
            }
            std::cout << ans << " ";
        }
...
int main()
{
    ...
    for (int i {1}; i <= t; ++i)
    {
        debug_(std::to_string(i) + " th test case:");
        shimuguyue.solve();
    }
