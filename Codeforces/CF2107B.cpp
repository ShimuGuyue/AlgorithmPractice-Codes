...
public:
	static void Solve(void)
	{
		...
		vector<int> as(n);
		cin >> as;
		int max = *max_element(as.begin(), as.end());
		int min = *min_element(as.begin(), as.end());

		int count_max = std::count_if(as.begin(), as.end(),
						[&max](int &x)
						{
							return x == max;
						});
		if (count_max == 1 && max - 1 - min > k)
		{
			cout << "Jerry" << endl;
			return;
		}
		if (count_max > 1 && max - min > k)
		{
			cout << "Jerry" << endl;
			return;
		}
