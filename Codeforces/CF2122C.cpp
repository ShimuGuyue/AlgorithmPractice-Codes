...
// solution
	void solve()
	{
		...
		sort(points.begin() + 1, points.end(), [](Edge a, Edge b)
		{
			return a.u < b.u;
		});
		sort(points.begin() + 1, points.begin() + 1 + n / 2, [](Edge a, Edge b)
		{
			return a.v < b.v;
		});
		sort(points.begin() + 1 + n / 2, points.end(), [](Edge a, Edge b)
		{
			return a.v < b.v;
		});
		for (int i = 1; i <= n / 2; ++i)
		{
			cout << points[i].w << " " << points[n - i + 1].w << endl;;
		}
	}
};
#pragma endregion
