...
public:
	static void Solve(void)
	{
		...
		for (int i = 1; i < n; ++i)
		{
			dp[i] = std::max(dp[i - 1] + as[i], int64_t(as[i]));
		...
		if (std::count(s.begin(), s.end(), '0') == 0 && max != k)
		{
			cout << "No" << endl;
			return;
		}
		...
		for (int i = 0; i < n; ++i)
		{
			...
			int l = i - 1;
			int64_t dpl = 0, maxSum_l = 0;
			while (l >= 0 && s[l] == '1')
			{
				dpl += as[l];
				maxSum_l = std::max(maxSum_l, dpl);
				--l;
			}
			int r = i + 1;
			int64_t dpr = 0, maxSum_r = 0;
			while (r < n && s[r] == '1')
			{
				dpr += as[r];
				maxSum_r = std::max(maxSum_r, dpr);
				++r;
			}
			as[i] = k - maxSum_l - maxSum_r;
			cout << as << endl;
			return;
		}
		cout << as << endl;
