...
public:
	static void Solve()
	{
		constexpr int64_t mod = INT64_C(998244353);
		...
		for (int i = 0; i < n; ++i)
		{
			...
			int64_t ans;
			if (maxa > maxb)
				ans = (quickPow(2, maxa, mod) + quickPow(2, bs[i - indexa], mod)) % mod;
			else if (maxb > maxa)
				ans = (quickPow(2, maxb, mod) + quickPow(2, as[i - indexb], mod)) % mod;
			else
				ans = (quickPow(2, maxa, mod) + quickPow(2, std::max(bs[i - indexa], as[i - indexb]), mod)) % mod;
			cout << ans << " ";
		}
		cout << endl;
	}
