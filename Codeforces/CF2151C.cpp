...
// solution
	void solve()
	{
		int n;
		cin >> n;
		vector<int64_t> as(n * 2 + 1);
		for (int i = 1; i <= n * 2; ++i)
		{
			cin >> as[i];
		}

		vector<int64_t> presum_full(n + 1);
		for (int i = 1; i <= n; ++i)
		{
			presum_full[i] = presum_full[i - 1] + as[n * 2 - i + 1] - as[i];
		}

		vector<int64_t> presum_out(n + 1);
		for (int i = 1; i <= n; ++i)
		{
			presum_out[i] = presum_out[i - 1] + as[i * 2] - as[i * 2 - 1];
		}

		vector<int64_t> presum_in(n);
		for (int i = 1; i < n; ++i)
		{
			presum_in[i] = presum_in[i - 1] + as[i * 2 + 1] - as[i * 2];
		}
		...
		for (int i = 2; i <= n; ++i)
		{
			...
			if (i & 1)
			{
				int l = i / 2 + 1;
				int r = n - l + 1;
				ans += presum_out[r] - presum_out[l - 1];
			}
			else
			{
				int l = i / 2;
				int r = n - l;
				ans += presum_in[r] - presum_in[l - 1];
			}
			cout << " " << ans;
		}
		cout << endl;
