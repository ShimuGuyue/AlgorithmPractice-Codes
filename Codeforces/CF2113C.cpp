...
public:
	static void Solve()
	{
		...
		for (int i = 1; i <= n; ++i)
		{
			...
			{
				cin >> grid[i][j];
				pres[i][j] = pres[i-1][j] + pres[i][j-1] - pres[i-1][j-1] + (grid[i][j] == 'g');
			}
		}
		int sum = pres[n][m];
		int ans = 0;
		for (int i = 1; i <= n; ++i)
		{
			for (int j = 1; j <= m; ++j)
			{
				if (grid[i][j] != '.')
					continue;
				int x1 = std::max(i - k + 1, 1);
				int y1 = std::max(j - k + 1, 1);
				int x2 = std::min(i + k - 1, n);
				int y2 = std::min(j + k - 1, m);
				int count = pres[x2][y2] - pres[x2][y1-1] - pres[x1-1][y2] + pres[x1-1][y1-1];
