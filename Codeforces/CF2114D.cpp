...
public:
	static void Solve()
	{
		...
		{
			cin >> locs[i];
			xs.insert(locs[i].first);
			ys.insert(locs[i].second);
		}
		if (n == 1)
		{
			cout << 1 << endl;
		...
		for (auto [x, y] : locs)
		{
			...
			int miny = *ys.begin();
			int maxy = *ys.rbegin();
			int64_t lenx = maxx - minx + 1;
			int64_t leny = maxy - miny + 1;
			if (lenx * leny == n - 1)
				ans = std::min({ans, (lenx + 1) * leny, lenx * (leny + 1)});
			else
				ans = std::min(ans, lenx * leny);
			xs.insert(x);
			ys.insert(y);
		}
		cout << ans << endl;
