...
// solution
	void solve()
	{
		...
		bool ok = false;
		ans = "L";
		dfs(1, n-1, as[0], 0, ok, ans, as);
		ans = "R";
		dfs(0, n-2, as[n-1], 0, ok, ans, as);
	}

	void dfs(int l, int r, int last, int flag, bool &ok, string &ans, vector<int> &as)
	{
		// cout << as[l] << " " << as[r] << " " << last << " " << flag << endl;
		if (ok)
			return;
		if (l == r)
		{
