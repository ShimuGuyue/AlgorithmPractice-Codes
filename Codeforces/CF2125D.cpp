...
// 欧拉筛
pair<vector<int>, vector<bool>> Euler(int n)
{
	vector<int> primes;
...
// solution
	void solve()
	{
		...
		dp[0] = 1;
		vector<vector<Segement>> segements(m + 1);	// 按右端点分桶
		for (int i = 0; i < n; ++i)
		{
			int l, r, p, q;
			cin >> l >> r >> p >> q;
			int64_t p_inv = p * inv(q, mod) % mod;
			segements[r].push_back({l, r, p_inv});
			dp[0] = dp[0] * (1 - p_inv + mod) % mod;
		}

		for (int i = 1; i <= m; ++i)
		{
			for (auto [l, r, p_inv] : segements[i])
			{
				int64_t temp = dp[l - 1];
				temp = temp * inv(1 - p_inv + mod, mod) % mod;
				temp = temp * p_inv % mod;
				dp[i] = (dp[i] + temp) % mod;
			}
		}
