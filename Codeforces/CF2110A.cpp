...
public:
	static void Solve()
	{
		...
		for (int i = 0; i < n; ++i)
		{
			if (first_even == -1 && as[i] % 2 == 0)
				first_even = i;
			if (first_odd == -1 && as[i] % 2 == 1)
				first_odd = i;
		}
		for (int i = n - 1; i >= 0; --i)
		{
			if (last_even == -1 && as[i] % 2 == 0)
				last_even = i;
			if (last_odd == -1 && as[i] % 2 == 1)
				last_odd = i;
		}

		int ans = n;
		if (as.front() % 2 == 0)
			ans = std::min(ans, n - 1 - last_even);
		else
			ans = std::min(ans, n - 1 - last_odd);
		if (as.back() % 2 == 0)
			ans = std::min(ans, first_even);
		else
			ans = std::min(ans, first_odd);
		cout << ans << endl;
	}
