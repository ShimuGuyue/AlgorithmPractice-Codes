...
// solution
	void solve()
	{
		...
		vector<int> as(n);
		cin >> as;
		int count_0 = std::count(as.begin(), as.end(), 0);
		int count_nega = std::count(as.begin(), as.end(), -1);
		int count_posi = std::count(as.begin(), as.end(),  1);

		int ans = count_0 + count_nega % 2 * 2;
		cout << ans << endl;
	}
};
#pragma endregion
