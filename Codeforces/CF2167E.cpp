...
// solution
	...
	void solve()
	{
		...
		maxHeap<Data> h;
		if (as.front() != 0)
			h.push({as.front() - 0, 0, 2});
		if (as.back() != x)
			h.push({x - as.back(), x, 0});
		for (int i = 1; i < n; ++i)
		{
			int len = as[i] - as[i - 1];
			if (len == 1)
			...
			if (len & 1)
			{
				int mid1 = (as[i] + as[i - 1]) / 2;
				h.push({mid1 - as[i - 1], mid1, 0});
				h.push({mid1 - as[i - 1], mid1 + 1, 2});
			}
			else
			{
				int mid = (as[i] + as[i - 1]) / 2;
				h.push({as[i] - mid, mid, 1});	
			}
		}
		...
		{
			--k;
			auto [len, loc, mode] = h.top();
			h.pop();
			visited.insert(loc);
			if (mode == 0 && len != 1)
			{
				h.push({len - 1, loc - 1, 0});
			}
			if (mode == 2 && len != 1)
			{
				h.push({len - 1, loc + 1, 2});
			}
			if (mode == 1 && len != 1)
			{
				h.push({len - 1, loc - 1, 0});
