...
// solution
	void solve()
	{
		...
		vector<int64_t> presums(n);
		partial_sum(as.begin(), as.end(), presums.begin());
		...
		while (q--)
		{
			...
			else
			{
				...
				if (r >= l)
				{
					cout << presums[r] - (l == 0 ? 0 : presums[l - 1]) << endl;
				}
				else
				{
					cout << presums[r] + presums.back() - (l == 0 ? 0 : presums[l - 1]) << endl;
				}
			}
		}
	}
