...
public:
	static void Solve(void)
	{
		...
		Dfs(1, ans, 0, n, m, cs, animals, counts);
		cout << ans << endl;
	}

	static void Dfs(int index, int64_t &ans, int64_t sum, int &n, int &m,
					vector<int> &cs, vector<vector<int>> &animals, vector<int> counts)
	{
		...
		Dfs(index + 1, ans, sum, n, m, cs, animals, counts);
		for (int i : animals[index])
		{
			++counts[i];
		}
		Dfs(index + 1, ans, sum + cs[index], n, m, cs, animals, counts);
		for (int i : animals[index])
		{
			++counts[i];
		}
		Dfs(index + 1, ans, sum + cs[index] * 2, n, m, cs, animals, counts);
	}
