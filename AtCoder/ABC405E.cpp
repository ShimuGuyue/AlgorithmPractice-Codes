...
public:
	static void Solve(void)
	{
		...
		fact.resize(a + b + c + d + 1);
		infact.resize(a + b + c + d + 1);
		fact.front() = 1;
		for (int i = 1; i <= a + b + c + d; ++i)
		{
			fact[i] = fact[i - 1] * i % mod;
		}
		infact.back() = Q_Pow(fact.back(), mod - 2) % mod;
		...
		for (int i = 0; i <= c; ++i)
		{
			ans = (ans + (C(a + i + b, b)) % mod * C(c - i + d - 1, d - 1) % mod) % mod;
		}
		cout << ans << endl;
	}

	static int64_t C(int n, int m)
	{
		return fact[n] * infact[m] % mod * infact[n - m] % mod;
	}

	static int64_t Q_Pow(int64_t base, int n)
	{
		if (base == 1)
			return 1;
		int64_t ans = 1;
