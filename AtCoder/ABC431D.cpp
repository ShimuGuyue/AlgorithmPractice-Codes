...
// solution
    ...
    void solve()
    {
        constexpr int m {500 * 500};

        int n;
        std::cin >> n;
        std::vector<Data> datas(n);
        for (auto &[w, h, b]: datas)
        {
            std::cin >> w >> h >> b;
        }
        ++datas;

        std::vector<int64_t> dp0(m * 2 + 1, INT64_MIN);
        std::vector<int64_t> dp1(m * 2 + 1, INT64_MIN);
        dp0[m] = 0;
        for (int i = 1; i <= n; ++i)
        {
            auto [w, h, b] {datas[i]};
            ...
            {
                if (j + w <= m * 2)
                    dp1[j] = std::max(dp1[j], dp0[j + w] + h);
                if (j - w >= 0)
                    dp1[j] = std::max(dp1[j], dp0[j - w] + b);
            }
            dp0 = dp1;
        }
        int64_t ans {0};
        for (int i {m + 0}; i <= m + m; ++i)
        {
            ans = std::max(ans, dp0[i]);
        }
        std::cout << ans << endl;
    }
};
