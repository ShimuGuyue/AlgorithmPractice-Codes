...
public:
	static void Solve()
	{
		...
		if (count_if(as.begin(), as.end(), [&as](int64_t n){return llabs(n) == llabs(as.front());}) == n)
		{
			int count_posi = count_if(as.begin(), as.end(), [](int64_t n){return n > 0;});
			int count_nega = count_if(as.begin(), as.end(), [](int64_t n){return n < 0;});
			if ((count_posi && count_nega) && abs(count_posi - count_nega) > 1)
				cout << "No" << endl;
			else
				cout << "Yes" << endl;
			return;
		...
		sort(as.begin(), as.end(), [](int64_t a, int64_t b){return llabs(a) < llabs(b);});
