...
public:
	...
	static void Resort(vector<int> &as, int l, int r)
	{
		if (l == r)
			return;
		int mid = (l + r) / 2;
		int minl = *std::min_element(as.begin() + l, as.begin() + mid + 1);
		int minr = *std::min_element(as.begin() + mid + 1, as.begin() + r + 1);
		if (minl > minr)
			std::reverse(as.begin() + l, as.begin() + r + 1);
		Resort(as, l, mid);
		Resort(as, mid + 1, r);
	}
