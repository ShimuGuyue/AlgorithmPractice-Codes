...
// solution
    void solve()
    {
        ...
        int maxindex = std::max_element(as.begin(), as.end()) - as.begin();
        std::vector<int> premaxs_l(n);
        std::vector<int> premaxs_r(n);
        for (int i = (maxindex - 1 + n) % n; i != maxindex; i = (i - 1 + n) % n)
        {
            premaxs_l[i] = std::max(premaxs_l[(i + 1) % n], as[i]);
        }
        for (int i = (maxindex + 1) % n; i != maxindex; i = (i + 1) % n)
        {
            premaxs_r[i] = std::max(premaxs_r[(i - 1 + n) % n], as[i]);
        }

        int64_t ans = 0;
        for (int i = (maxindex + 2) % n; i != (maxindex - 1 + n) % n; i = (i + 1) % n)
        {
            ans = std::max(ans, int64_t(as[maxindex] + as[i]) * (premaxs_r[(i - 1 + n) % n] + premaxs_l[(i + 1) % n]));
        }
        std::cout << ans << endl;
    }
};
