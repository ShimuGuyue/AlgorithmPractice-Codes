...
public:
	static void Solve(void)
	{
		...
		vector<int> as(n);
		cin >> as;
		int ans = Merge_Sort(as, 0, n - 1);
		cout << ans << endl;
	}

	static int Merge_Sort(vector<int> &as, int l, int r)
	{
		if (l == r)
			return 0;
		int mid = (l + r) / 2;
		int max = std::max(Merge_Sort(as, l, mid), Merge_Sort(as, mid + 1, r));
