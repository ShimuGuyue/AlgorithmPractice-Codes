...
// solution
	void solve()
	{
		...
		// for (int a : cs) cout << a << " "; cout << endl;
		// for (int b : ds) cout << b << " "; cout << endl;
		if (cs != ds)
		{
			cout << -1 << endl;
			return;
		...
		// for (int a : counts_a) cout << a << " "; cout << endl;
		// for (int b : counts_b) cout << b << " "; cout << endl;

		set<pair<int, int>> t;
		for (int i = 0; i < counts_a.size(); ++i)
		{
			if (counts_a[i] != counts_b[i])
				t.insert({counts_a[i], counts_b[i]});
		}

		// for (auto [k, v] : t) debug_(k, v);
		int ans = 0;
		for (auto [a, b] : t)
		{
			int64_t aa = a;
