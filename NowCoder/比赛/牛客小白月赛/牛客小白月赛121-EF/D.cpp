...
// solution
	void solve()
	{
		...
		int64_t sum_add = accumulate(as.begin(), as.end(), INT64_C(0));
		int64_t sum_xor = accumulate(as.begin(), as.end(), INT64_C(0), [](int64_t a, int64_t b)
		{
			return a ^ b;
		});

		int64_t ans = sum_add - sum_xor;
		cout << ans << endl;
	}
};
#pragma endregion
