	...
	static void Solve()
	{
		...
		for (int i = 0; i < n; ++i)
		{
			for (int j = 0; j < n; ++j)
			{
				...
				int aj = as[j];
				int aj_ = aj - 1;
				t.erase({counts[ai], ai});
				t.erase({counts[ai_], ai_});
				t.erase({counts[aj], aj});
				t.erase({counts[aj_], aj_});
				--counts[ai];
				++counts[ai_];
				--counts[aj];
				++counts[aj_];
				t.insert({counts[ai], ai});
				t.insert({counts[ai_], ai_});
				t.insert({counts[aj], aj});
				t.insert({counts[aj_], aj_});

				ans.insert(t.rbegin()->second);
