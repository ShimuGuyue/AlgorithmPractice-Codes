...
// solution
	void solve()
	{
		...
		map<int, int> count_presums;
		vector<vector<bool>> visited(n, vector<bool>(m));

		minHeap<Node> h;
		for (int i = 0; i < m; ++i)
		{
			h.push(Node{0, i, grid[0][i]});
			visited[0][i] = true;
		}
		...
		while (!h.empty())
		{
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				if (visited[xx][yy])
					continue;
				visited[xx][yy] = true;
				h.push(Node{xx, yy, grid[xx][yy]});
			}
		}

		for (auto it = count_presums.begin(), jt = std::next(it); jt != count_presums.end(); ++jt, ++it)
		{
			jt->second += it->second;
		}
		...
		while (q--)
		{
			int x;
			cin >> x;
			auto it = std::prev(count_presums.upper_bound(x));
			cout << it->second << endl;
		}
	}
};
