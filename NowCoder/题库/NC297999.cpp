...
// solution
	void solve()
	{
		...
		cin >> base >> c >> mod;
		vector<int> flags{0};
		unordered_map<int64_t, int> visited;
		for (int i = 1; i <= 1e6; ++i)
		{
			c = quickPow(base, c, mod);
			if (visited[c])
				break;
			visited[c] = i;
			flags.push_back(c);
		...
		vector<int> pres(flags.size());
		partial_sum(flags.begin(), flags.end(), pres.begin(), [mod](int a, int b)
		{
			return (a + b) % mod;
		});

		int n = pres.size();
		int front = visited[c] - 1;
		int len = n - 1 - front;
		...
		while (q--)
		{
			...
			int64_t ans = pres[front];
			int64_t count = (k - front) / len;
			ans = (ans + count % mod * (pres[n - 1] - pres[front] + mod) % mod) % mod;
			k = (k - front) % len + front;
			ans = (ans + pres[k] - pres[front] + mod) % mod;
			cout << ans << endl;
		}
	}
};
