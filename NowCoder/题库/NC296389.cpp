...
public:
	static void Solve()
	{
		...
		map<int, int> maxlen_equal;
		map<int, int> maxlen_mutil;
		int ans = 0;
		int max = 1;
		vector<int> dp(n + 2, 1);
		for (int r = 0; r <= n + 1; ++r)
		{
			...
			else
			{
				...
				for (int i = 1; i * i <= as[r]; ++i)
				{
					if (as[r] % i)
						continue;
					max1 = std::max(max1, maxlen_equal[i]);
					max1 = std::max(max1, maxlen_equal[as[r] / i]);
				}
				// >
				max1 = std::max(max1, maxlen_mutil[as[r]]);

				dp[r] = std::max(dp[r], max1 + 1);
				max = std::max(max, dp[r]);

				for (int i = 1; i * i <= as[r]; ++i)
				{
					if (as[r] % i)
						continue;
					maxlen_mutil[i] = std::max(maxlen_mutil[i], dp[r]);
					maxlen_mutil[as[r] / i] = std::max(maxlen_mutil[as[r] / i], dp[r]);
				}
				maxlen_equal[as[r]] = std::max(maxlen_equal[as[r]], dp[r]);
			}
		}
		cout << ans - 2 << endl;
	}
