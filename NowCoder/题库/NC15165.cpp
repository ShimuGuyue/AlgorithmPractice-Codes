...
public:
	static void Solve()
	{
		...
		cin >> s;
		int n = s.length();
		vector<int> nexts = Build_nexts(s);
		...
		for (int i = nexts[n-1]; i < n-1; ++i)
		{
			if (nexts[i] == nexts[n-1])
			{
				cout << s.substr(0, nexts[n-1]) << endl;
				return;
			}
		}

		if (nexts[nexts[n-1] - 1] > 0)
		{
			cout << s.substr(0, nexts[nexts[n-1] - 1]) << endl;
			return;
		}
	...
	static vector<int> Build_nexts(string &s)
	{
		int n = s.length();
		vector<int> nexts(n);
		for (int i = 1; i < n; ++i)
