...
public:
	static void Solve()
	{
		...
		vector<vector<int64_t>> dp(101, vector<int64_t>(n + 1));
		for (int p = 1; p <= 100; ++p)
		{
			for (int i = 1; i <= n; ++i)
			{
				dp[p][i] = (((1 + dp[p][i-1]) * 100 % mod) * Pow(p, mod - 2, mod) % mod - dp[p][i-1] + mod) % mod;
			}
		}
		vector<vector<int64_t>> pres(101, vector<int64_t>(n + 1));
		for (int p = 1; p <= 100; ++p)
		{
			for (int i = 1; i <= n; ++i)
			{
				pres[p][i] = pres[p][i-1] + dp[p][i] % mod;
			}
		}
