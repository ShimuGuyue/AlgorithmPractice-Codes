...
public:
	static void Solve(void)
	{
		...
		vector<int> first_indexs{0, -1, -1, -1, -1, -1, -1};
		vector<int> last_indexs{-1, -1, -1, -1, -1, -1, -1};
		...
		for (int i = 1; i < 7; ++i)
		{
			ans = std::max(ans, last_indexs[i] - first_indexs[i]);
		}
		ans = std::max(ans, last_indexs[0] - first_indexs[0] + 1);
		cout << ans << endl;
	}
