...
// solution
    void solve()
    {
        ...
        std::vector<std::vector<int>> dp(n + 1, std::vector<int>(n + 1)); // 区间长度 - 起始位置
        for (int len = 2; len <= n; ++len)
        {
            for (int l = 1; l + len - 1 <= n; ++l)
            {
                int cur = INT_MAX;
                int sum = presums[l + len - 1] - presums[l - 1];
                for (int len1 = 1; len1 < len; ++len1)
                {
                    int r = l + len1;
                    int len2 = len - len1;
                    cur = std::min(cur, dp[len1][l] + dp[len2][r] + sum);
                }
                dp[len][l] = cur;
            }
        }
