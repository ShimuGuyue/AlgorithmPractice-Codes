...
// solution
	...
	void solve()
	{
		int n, kk;
		cin >> n >> kk;
		vector<vector<int64_t>> grid(n + 1, vector<int64_t>(n + 1));
		for (int i = 1; i <= n; ++i)
		{
			for (int j = 1; j <= i; ++j)
			{
		...
		vector<vector<vector<int64_t>>> dp(n + 1, vector<vector<int64_t>>(n + 1, vector<int64_t>(kk + 1, -1e18)));
		dp[0][0][0] = 0;
		for (int i = 1; i <= n; ++i)
		{
			for (int j = 1; j <= i; ++j)
			{
				for (int k = 0; k <= std::min(i, kk); ++k)
				{
					dp[i][j][k] = std::max(dp[i - 1][j - 1][k], dp[i - 1][j][k]) + grid[i][j];
					if (k > 0)
						dp[i][j][k] = std::max({
							dp[i][j][k],
							dp[i - 1][j - 1][k - 1] + grid[i][j] * 3,
							dp[i - 1][j][k - 1] + grid[i][j] * 3, 
						});
				}
			}
		}
