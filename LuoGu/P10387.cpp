...
public:
	static void Solve()
	{
		int64_t n, s;
		cin >> n >> s;
		vector<pair<int64_t, int>> pcs(n);
		cin >> pcs;

		sort(pcs.begin(), pcs.end(), [](pair<int, int> a, pair<int, int> b)
		{
			return a.second < b.second;
		});

		vector<int64_t> sums_suf(n);
		sums_suf[n - 1] = pcs[n - 1].first;
		for (int i = n - 2; i >= 0; --i)
		{
			sums_suf[i] = sums_suf[i + 1] + pcs[i].first;
		}
