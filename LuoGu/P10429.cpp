...
public:
	static void Solve()
	{
		...
		vector<int64_t> sums_pre(n + 1);
		for (int i = 1; i <= n; ++i)
		{
			sums_pre[i] = sums_pre[i - 1] + as[i];
		}

		using Range = pair<int64_t, pair<int, int>>;
		vector<Range> sums_ranges;
		for (int i = 1; i <= n; ++i)
		{
			for (int j = i; j <= n; ++j)
			{
				sums_ranges.push_back({sums_pre[j] - sums_pre[i - 1], {i, j}});
			}
		}

		int64_t ans = 1e18;
		sort(sums_ranges.begin(), sums_ranges.end());
		for (int i = 1; i < sums_ranges.size(); ++i)
		{
			if (sums_ranges[i].second.first < sums_ranges[i - 1].second.first
			&&	sums_ranges[i].second.second >= sums_ranges[i - 1].second.first)
				continue;
			if (sums_ranges[i].second.first > sums_ranges[i - 1].second.first
			&&	sums_ranges[i].second.first <= sums_ranges[i - 1].second.second)
				continue;
			ans = std::min(ans, sums_ranges[i].first - sums_ranges[i - 1].first);
		}
		cout << ans << endl;
	}
