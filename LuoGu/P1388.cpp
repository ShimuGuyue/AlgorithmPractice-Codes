...
public:
	static void Solve(void)
	{
		...
		for (int i = 1; i <= n; ++i)
		{
			for (int j = i + 1; j <= n; ++j)
			{
				for (int m = 1; m <= std::min(k, j - i); ++m)
				{
					// 加法连接左右两部分
					for (int c = 0; c <= m; ++c)
					{
						// 左半部分乘号数
						for (int t = i + c; t <= j - (m - c) - 1; ++t)
						{
							// 左半部分终点
							dp[i][j][m] = std::max(dp[i][j][m], dp[i][t][c] + dp[t + 1][j][m - c]);
						}
					}
					...
					for (int c = 0; c <= m - 1; ++c)
					{ 
						for (int t = i + c; t <= j - (m - 1 - c) - 1; ++t)
						{
							dp[i][j][m] = std::max(dp[i][j][m], dp[i][t][c] * dp[t + 1][j][m - 1 - c]);
						}
					}
				}
			}
