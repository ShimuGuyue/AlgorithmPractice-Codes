...
public:
	static void Solve()
	{
		int n, m, c;
		cin >> n >> m >> c;
		vector<vector<int>> gird(n + 1, vector<int>(m + 1));
		for (int i = 1; i <= n; ++i)
		{
			for (int j = 1; j <= m; ++j)
			{
		...
		vector<vector<int64_t>> presums(n + 1, vector<int64_t>(m + 1));
		for (int i = 1; i <= n; ++i)
		{
			for (int j = 1; j <= m; ++j)
			{
				presums[i][j] = gird[i][j] + presums[i - 1][j] + presums[i][j - 1] - presums[i - 1][j - 1];
			}
		}
		...
		for (int i = c; i <= n; ++i)
		{
			for (int j = c; j <= m; ++j)
			{
				int64_t sum = presums[i][j] - presums[i - c][j] - presums[i][j - c] + presums[i - c][j - c];
				if (sum > max)
				{
					max = sum;
					x = i - c + 1;
