...
public:
	static void Solve(void)
	{
		int n, m, q;
		cin >> n >> m >> q;
		vector<vector<uint64_t>> grid(n + 1, vector<uint64_t>(m + 1, 0));
		for (int i = 1; i <= n; ++i)
		{
			for (int j = 1; j <= m; ++j)
			{
		...
		vector<vector<uint64_t>> sums(n + 1, vector<uint64_t>(m + 1, 0));
		{
			for (int i = 1; i <= n; ++i)
			{
				for (int j = 1; j <= m; ++j)
		...
		while (q--)
		{
			int u, v, x, y;
			cin >> u >> v >> x >> y;
			uint64_t sum = sums[x][y] - sums[x][v - 1] - sums[u - 1][y] + sums[u - 1][v - 1];
			ans ^= sum;
		}
		cout << ans << endl;
	}
