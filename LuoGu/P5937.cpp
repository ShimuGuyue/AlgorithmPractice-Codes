...
public:
	static void Solve()
	{
		...
		DisjointSet ds(n * 2 + 1);	// 维护每个位置的前缀奇数个数的奇偶性关系
		for (int i = 1; i <= m; ++i)
		{
			auto [l, r, op] = orders[i];
			int len = r - l + 1;
			l = lower_bound(indexs.begin(), indexs.end(), l - 1) - indexs.begin();
			r = lower_bound(indexs.begin(), indexs.end(), r) - indexs.begin();

			if (op == "even")
			{
				if (ds.In_same_set(l + n, r))
				{
					cout << i - 1 << endl;
					return;
				}
				...
				{
					ds.Merge_set(l, r);
					ds.Merge_set(l + n, r + n);
				}
			}
			else
			{
				if (ds.In_same_set(l, r))
				{
					cout << i - 1 << endl;
					return;
				}
