...
// solution
	...
	void solve()
	{
		...
		vector<vector<pair<int, int>>> dp(n + 1, vector<pair<int, int>>(m + 1, {INT_MIN, 0})); // max, last
		fill(dp[0].begin(), dp[0].end(), pair<int, int>{0, 0});
		for (int i = 1; i <= n; ++i)
		{
			// dp[i][0] = {0, 0};
			for (int j = i; j <= m; ++j)
			{
				for (int k = i - 1; k < j; ++k)
				{
					if (dp[i - 1][k].first + as[i][j] > dp[i][j].first)
					{
						dp[i][j] = {dp[i - 1][k].first + as[i][j], k};
					}
				}
			}
		}
