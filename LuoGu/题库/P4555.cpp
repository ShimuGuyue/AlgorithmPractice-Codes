...
// struct
	...
	{};
// init
	ShimuGuyue() : MULTIPLE_TESTS(false)
	{}
// solution
	void solve()
	{
		...
		turn(s);
		int n = s.size();
		vector<int> lens = manacher(s);

		vector<int> ls(n);	// 从某点向左的最长回文子串
		vector<int> rs(n);	// 从某点向右的最长回文子串
		for (int i = 0; i < n; ++i)
		{
			int l = i - lens[i] + 1;
			int r = i + lens[i] - 1;
			int len = lens[i] * 2 - 1;
			ls[r] = std::max(ls[r], len);
			rs[l] = std::max(rs[l], len);
		}
		for (int i = 2; i < n - 2; i += 2)
		{
			rs[i] = std::max(rs[i], rs[i-2] - 4);
		}
		for (int i = n - 3; i >= 2; i -= 2)
		{
			ls[i] = std::max(ls[i], ls[i+2] - 4);
		}

		int ans = 0;
		for (int i = 2; i < n - 2; i += 2)
		{
			int len = (ls[i] + rs[i]) / 2 - 1;
			ans = std::max(ans, len);
		}
		cout << ans << endl;
	}
	...
	vector<int> manacher(const string &s)
	{
		...
		for (int i = 0; i < n; ++i)
		{
			...
			else
			{
				...
				{
					lens[i] = r - i + 1;
					for (int len = lens[i]; i - len >= 0 && i + len < n; ++len)
					{
						if (s[i-len] != s[i+len])
							break;
						lens[i] = len + 1;
