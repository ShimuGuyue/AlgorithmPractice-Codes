...
public:
	static void Solve()
	{
		...
		vector<vector<int>> presums(n + 1, vector<int>(m + 1));
		for (int i = 1; i <= n; ++i)
		{
			for (int j = 1; j <= m; ++j)
			{
				presums[i][j] = grid[i][j] + presums[i-1][j] + presums[i][j-1] - presums[i-1][j-1];
			}
		}

		auto Check = [&n, &m, &a, &b, &presums](int k) -> bool
		{
			int count_r = 0;
			int last_i = 0;
			for (int i = 1; i <= n; ++i)
			{
				if (presums[i][m] - presums[last_i][m] < k * b)
					continue;
				int count_c = 0;
				int last_j = 0;
				for (int j = 1; j <= m; ++j)
				{
					if (presums[i][j] - presums[i][last_j] - presums[last_i][j] + presums[last_i][last_j] < k)
						continue;
					++count_c;
					last_j = j;
				}
