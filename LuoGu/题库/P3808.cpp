...
public:
	void insert(const string &s)
	{
		...
		++node->count_end;
		node->end_id.push_back(dictionary.size());
	...
	uint32_t match(const string &text)
	{
		if (!isbuild)
			build();
		vector<bool> matched(dictionary.size());
		Node* node = root;
		for (const char c : text)
		{
			...
			node = node->nexts[index];
			// ans += node->count_end;
			for (size_t id : node->end_id)
			{
				matched[id] = true;
			}
		}
		return accumulate(matched.begin(), matched.end(), 0);
	}
};
