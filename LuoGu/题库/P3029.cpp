...
public:
	static void Solve()
	{
		...
		set<int> ids;
		for (auto [loc, id] : cows)
		{
			ids.insert(id);
		}
		int count_id = ids.size();
		...
		while (r < n)
		{
			auto [loc_r, id_r] = cows[r++];
			++counts[id_r];
			if (counts.size() < count_id)
				continue;
			while (counts[cows[l].second] > 1)
			{
				--counts[cows[l].second];
				++l;
			}
			ans = std::min(ans, loc_r - cows[l].first);
		}
		cout << ans << endl;
	}
