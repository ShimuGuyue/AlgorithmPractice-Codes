...
// solution
	...
	void solve()
	{
		using Item = pair<int, int>;	// value - cost
		...
		int max = max_element(items.begin() + 1, items.end(),
		[](Item a, Item b)
		{
		...
		vector<vector<int>> dp(n + 1, vector<int>(m + max + 1, INT_MAX));
		dp[0][0] = 0;
		for (int i = 1; i <= n; ++i)
		{
			auto [v, c] = items[i];
			...
			{
				dp[i][j] = dp[i - 1][j];
				if (j - v >= 0 && dp[i][j - v] != INT_MAX)
					dp[i][j] = std::min(dp[i][j], dp[i][j - v] + c);
			}
		}
		int ans = *min_element(dp[n].begin() + m, dp[n].end());
		cout << ans << endl;
	}
};
#pragma endregion
