...
public:
	static void Solve()
	{
		...
		vector<int64_t> dp_max_l(n, 0);
		for (int i = 1; i < n; ++i)
		{
			dp_max_l[i] = std::max(int64_t(0), dp_max_l[i - 1] + as[i - 1]);
		}
		vector<int64_t> dp_max_r(n, 0);
		for (int i = n - 2; i >= 0; --i)
		{
			dp_max_r[i] = std::max(int64_t(0), dp_max_r[i + 1] + as[i + 1]);
		}

		for (int i = 0; i < n; ++i)
		{
			int64_t ans = dp_max_l[i] + as[i] + dp_max_r[i];
			cout << ans << " ";
		}
		cout << endl;
	}
