...
public:
	static void Solve(void)
	{
		...
		queue<BfsData> bfs;
		bfs.push({0, 0, vector<vector<int>>(5, vector<int>(5, 0))});
		int ans = 0;
		while (!bfs.empty())
		{
			auto [index, count1, grid] = bfs.front();
			bfs.pop();
			if (count1 == 12)
			{
				ans += Is_Draw(grid);
			...
			bfs.push({index + 1, count1, grid});
			grid[index / 5][index % 5] = 1;
			bfs.push({index + 1, count1 + 1, grid});
		}
		cout << ans << endl;
	}

	static bool Is_Draw(vector<vector<int>> &grid)
	{
		for (int i = 0; i < 5; ++i)
		{
			if (grid[i][0] == grid[i][1] && grid[i][0] == grid[i][2] && grid[i][0] == grid[i][3] && grid[i][0] == grid[i][4])
				return false;
			if (grid[0][i] == grid[1][i] && grid[0][i] == grid[2][i] && grid[0][i] == grid[3][i] && grid[0][i] == grid[4][i])
				return false;
		}
		if (grid[0][0] == grid[1][1] && grid[0][0] == grid[2][2] && grid[0][0] == grid[3][3] && grid[0][0] == grid[4][4])
			return false;
		if (grid[0][4] == grid[1][3] && grid[0][4] == grid[2][2] && grid[0][4] == grid[3][1] && grid[0][4] == grid[4][0])
			return false;
		return true;
	}
};
